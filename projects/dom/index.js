...
function findAllPSiblings(where) {
  ...
  for (const part of where.children) {
    if (part.nextElementSibling && part.nextElementSibling.tagName === 'P') {
      PSiblings.push(part);
    }
  }
...
function deleteTextNodes(where) {
  for (let i = 0; i < where.childNodes.length; i++) {
    const textel = where.childNodes[i];

    if (textel.nodeType === Element.TEXT_NODE) {
      where.removeChild(textel);
      i--;
    }
  }
}
...
function deleteTextNodesRecursive(where) {
  for (let i = 0; i < where.childNodes.length; i++) {
    ...
    if (textel.nodeType === Element.TEXT_NODE) {
      where.removeChild(textel);
      i--;
    } else if (textel.nodeType === Element.ELEMENT_NODE) {
      deleteTextNodesRecursive(textel);
    }
  }
}
